...
class MinHook(BaseChecker):
    VENDOR = "tsudakageyu"
    PRODUCT = "minhook"
    LINK_SOURCE = "https://github.com/TsudaKageyu/minhook"
    ...
    RX_MAJOR = re.compile(
        r"set\s*\(\s*MINHOOK_MAJOR_VERSION\s+(\d+)\s*\)",
        re.IGNORECASE,
    )
    RX_MINOR = re.compile(
        r"set\s*\(\s*MINHOOK_MINOR_VERSION\s+(\d+)\s*\)",
        re.IGNORECASE,
    )
    RX_PATCH = re.compile(
        r"set\s*\(\s*MINHOOK_PATCH_VERSION\s+(\d+)\s*\)",
        re.IGNORECASE,
    )
