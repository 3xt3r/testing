...
class Cppkafka(BaseChecker):
    VENDOR = "mfontanini"
    PRODUCT = "cppkafka"
    LINK_SOURCE = "https://github.com/mfontanini/cppkafka.git"

    STOP_AFTER_FIRST_VERSION = True

    CONTAINS_PATTERNS = [
        r"namespace\s+cppkafka\s*\{",
    ]

    RX_MAJOR = re.compile(
        r"set\s*\(\s*CPPKAFKA_VERSION_MAJOR\s+(\d+)\s*\)",
        re.IGNORECASE,
    )
    RX_MINOR = re.compile(
        r"set\s*\(\s*CPPKAFKA_VERSION_MINOR\s+(\d+)\s*\)",
        re.IGNORECASE,
    )
    RX_PATCH = re.compile(
        r"set\s*\(\s*CPPKAFKA_VERSION_REVISION\s+(\d+)\s*\)",
        re.IGNORECASE,
    )
