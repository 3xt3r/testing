...
class Minitrace(BaseChecker):
    VENDOR = "hrydgard"
    PRODUCT = "minitrace"
    LINK_SOURCE = "https://github.com/hrydgard/minitrace"
    ...
    RX_MAJOR = re.compile(
        r"set\s*\(\s*MTR_VERSION_MAJOR\s+(\d+)\s*\)",
        re.IGNORECASE,
    )
    RX_MINOR = re.compile(
        r"set\s*\(\s*MTR_VERSION_MINOR\s+(\d+)\s*\)",
        re.IGNORECASE,
    )
    RX_PATCH = re.compile(
        r"set\s*\(\s*MTR_VERSION_PATCH\s+(\d+)\s*\)",
        re.IGNORECASE,
    )
